...
//        jsonObject.getString("email");
   ...
   */
    ...
    public void process(Message message, @Headers Map<String,Object> headers, Channel channel) throws Exception {
        ...
        System.out.println("死信队列消费者获取生产者消息并根据信息发送邮件"+msg+"，获取的消息id为："+messageId);
        //  int i=1/0; TODO
        String sendEmailUrl="http://192.168.50.73:8083/sendEmail";
        System.out.println("死信队列消费者开始调用第三方邮件服务器：sendEmailUrl为"+sendEmailUrl);
        JSONObject result = HttpClientUtils.httpGet(sendEmailUrl);
        if (result==null){
            //重试 使用SpringAOP 拦截异常
            throw new Exception("网络访问不通，重试请求");
        }
        System.out.println("死信队列消费者调用第三方邮件服务器成功：result为"+result+"程序执行结束");
